...
function removeFirstOccurrences(str, value) {
  const firstIndexValue = str.indexOf(value);
  if (firstIndexValue === -1) return str;
  return (
    str.slice(0, firstIndexValue) + str.slice(firstIndexValue + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndexValue === -1) return str;
  return (
...
function isPalindrome(str) {
  const phrase = str.replace(/\W+/gi, '').toLowerCase();
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if ((code > 96 && code < 97 + 13) || (code > 64 && code < 65 + 13)) {
      rot13 += String.fromCharCode(code + 13);
    } else if (code < 64) {
      rot13 += str[i];
    } else {
      rot13 += String.fromCharCode(code - 13);
...
function getCardId(value) {
  ...
  return arrCardId.indexOf(value);
}
